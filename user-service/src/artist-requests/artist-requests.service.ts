...
import { ArtistRequest, ArtistRequestStatus } from './entities/artist-request.entity.js';
...
export class ArtistRequestsService {
  constructor(
    @InjectRepository(ArtistRequest) private requestsRepo: Repository<ArtistRequest>,
    @InjectRepository(ArtistRequestMessage) private messagesRepo: Repository<ArtistRequestMessage>,
  ...
  async submit(userId: number, content: string) {
    ...
    if (existing) throw new ConflictException('Vous avez déjà une demande en cours');
  ...
  async adminList() {
    const requests = await this.requestsRepo.find({ order: { updated_at: 'DESC' } });
    if (requests.length === 0) return [];

    const userIds = [...new Set(requests.map((r) => r.user_id))];
    const users = await this.usersRepo.find({ where: { id: In(userIds) } });
    const usersMap = Object.fromEntries(
      users.map((u) => [u.id, { id: u.id, firstname: u.firstname, lastname: u.lastname, email: u.email }]),
    );
  ...
  async getById(id: number) {
    const artistRequest = await this.requestsRepo.findOne({ where: { id }, relations: ['messages'] });
  ...
  async adminAddMessage(requestId: number, adminId: number, content: string) {
    const artistRequest = await this.requestsRepo.findOne({ where: { id: requestId } });
    if (!artistRequest) throw new NotFoundException('Demande introuvable');
    if (
  ...
  async adminDecide(requestId: number, adminId: number, action: 'approve' | 'reject') {
    const artistRequest = await this.requestsRepo.findOne({ where: { id: requestId } });
    if (!artistRequest) throw new NotFoundException('Demande introuvable');
    if (
      artistRequest.status === ArtistRequestStatus.APPROVED ||
    ...
    if (action === 'approve') {
      await this.usersRepo.update(artistRequest.user_id, { role: UserRole.ARTIST });
      artistRequest.status = ArtistRequestStatus.APPROVED;
    } else {
      artistRequest.status = ArtistRequestStatus.REJECTED;
    }
